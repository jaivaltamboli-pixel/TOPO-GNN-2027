...
class LLRResidualRanker(nn.Module):
    ...
    def forward(self, x6, edge_index, edge_attr, is_par, mask_diff, delta_w, num_defects, batch_map=None, num_graphs=None):
        ...
        return -delta_w + diff_energy + residual
...
def run_prior_calibrated_experiment():
    ...
    print("=" * 95)
    print("PHASE G.8: BAYESIAN PRIOR-CALIBRATED LLR RANKER")
    print("=" * 95 + "\n")
    ...
    print("\n[+] Sweeping Prior Log-Odds Shifts across Test Sets...\n")
    model.eval()

    # Prior offset sweep to match the 1000:1 physical base rate
    beta_sweep = [4.0, 5.0, 6.0, 7.0, 8.0, 9.0, 10.0, 12.0, 15.0]
    ...
    for d in test_distances:
        ...
        print(f"{'Prior Shift Beta':<18} | {'Topo P_L':<10} | {'Recoveries':<11} | {'Regressions':<12} | {'Net Gain'}")
        print("-" * 80)
        ...
        for beta in beta_sweep:
            ...
            for idx_k, (llr_val, obs_a, s_idx) in enumerate(zip(llr_arr, obs_A_list, shot_indices)):
                # Flip to opposite class only when LLR exceeds the physical base-rate prior shift
            ...
            print(f"Beta = {beta:<11.1f} | {topo_pl:6.3f}%   | {rec:>5d}/{mwpm_errs:<4d}  | {reg:>5d}/{shots_per_dist-mwpm_errs:<6d} | {net:>+5d}")
        print()

if __name__ == "__main__":
    run_prior_calibrated_experiment()
