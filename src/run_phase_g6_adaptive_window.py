...
class AdaptivePrecisionCycleRanker(nn.Module):
...
def run_adaptive_window_experiment():
    ...
    print("=" * 95)
    print("PHASE G.6: DISTANCE-SCALED AMBIGUITY WINDOW (20,000 shots/distance)")
    print("=" * 95 + "\n")
    ...
    model = AdaptivePrecisionCycleRanker(in_node_dim=6, in_edge_dim=4, hidden_dim=64, num_layers=4).to(device)
    ...
    print("\n[+] Evaluating Distance-Scaled Window Decision across Test Sets...\n")
    model.eval()

    k_window_sweep = [1.2, 1.5, 1.8, 2.2, 2.6]
    ...
    for d in test_distances:
        ...
        print(f"{'Window (k*d)':<14} | {'Eff Tau':<10} | {'Topo P_L':<10} | {'Recoveries':<11} | {'Regressions':<12} | {'Net Gain'}")
        print("-" * 80)

        for k in k_window_sweep:
            win_limit = k * d
            eff_tau = 3.5 * np.sqrt(d)
            topo_preds = preds_mwpm.copy()

            for idx_k, (e_val, dw_val, obs_a, s_idx) in enumerate(zip(e_arr, d_w_arr, obs_A_list, shot_indices)):
                if abs(dw_val) <= win_limit:
            ...
            print(f"<= {win_limit:<10.1f} | {eff_tau:<10.2f} | {topo_pl:6.3f}%   | {rec:>5d}/{mwpm_errs:<4d}  | {reg:>5d}/{shots_per_dist-mwpm_errs:<6d} | {net:>+5d}")
        print()

if __name__ == "__main__":
    run_adaptive_window_experiment()
