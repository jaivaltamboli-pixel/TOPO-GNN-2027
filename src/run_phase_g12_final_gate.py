...
class LengthNormalizedRanker(nn.Module):
    def __init__(self, in_node_dim=6, in_edge_dim=4, hidden_dim=64, num_layers=4):
        ...
        # Bounded edge scorer so evidence cannot explode
    ...
    def forward(self, x6, edge_index, edge_attr, is_par, mask_diff, delta_w, d_tens, batch_map=None, num_graphs=None):
        ...
        # Calibrated LLR directly pitting topological evidence against normalized classical gap
        llr_norm = - (delta_w / d_tens) + (mean_diff_energy * self.alpha)
        return llr_norm

def collate_cycle_batch(samples, device):
    x6_list, e_idx_list, e_attr_list, e_par_list, mask_diff_list = [], [], [], [], []
    batch_map, y_list, delta_w_list, d_list = [], [], [], []
    node_offset = 0
    ...
    # Class 1 correct -> label 1.0 (push LLR > 0). Class 0 correct -> label 0.0 (push LLR < 0).
    ...
    return bx6, be_idx, be_attr, be_par, bmask, delta_w, d_tens, bmap, labels, len(samples)
...
def collect_dataset(distances, p_val, eta, shots):
    ...
    for d in distances:
        ...
        for idx in active_indices:
            ...
            # Dictionary structure strictly prevents unpacking errors
            samples.append({
                "x6": x6,
                "e_idx": e_idx,
                "e_attr": e_attr,
                "e_par": (e_par.view(-1) > 0).float(),
                "mask_diff": mask_diff,
                "y_target": y_target,
                "delta_w": delta_w,
                "d_val": float(d),
...
def run_final_gate_experiment():
    ...
    print("=" * 95)
    print("PHASE G.12: BOUNDED PHYSICS RANKER (FINAL GATE 2 EVAL)")
    print("=" * 95 + "\n")
    ...
    for d in train_distances:
        for s in train_data[d]["samples"]:
            # Correct targets vs MWPM observations to mine failures
    ...
    model = LengthNormalizedRanker(in_node_dim=6, in_edge_dim=4, hidden_dim=64, num_layers=4).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            bx6, be_idx, be_attr, be_par, bmask, d_w, d_tens, bmap, labels, n_g = collate_cycle_batch(batch, device)

            optimizer.zero_grad()
            llr_norm = model(bx6, be_idx, be_attr, be_par, bmask, d_w, d_tens, batch_map=bmap, num_graphs=n_g)

            # Standard BCE with Logits - no crazy penalties, no unconstrained bias
        ...
        print(f"  Epoch {epoch:2d}/{epochs:2d} | BCE Loss: {total_loss/total_items:.4f} | Accuracy: {acc:6.2f}%")

    print("\n[+] Evaluating Bounded LLR Decisions across Test Sets...\n")
    model.eval()

    gamma_sweep = [0.0, 0.25, 0.5, 1.0, 1.5, 2.0]
    ...
    for d in test_distances:
        ...
        for i in range(0, len(active_samples), eval_batch_size):
            ...
            bx6, be_idx, be_attr, be_par, bmask, d_w, d_tens, bmap, _, n_g = collate_cycle_batch(batch_raw, device)

            with torch.no_grad():
                llr_out = model(bx6, be_idx, be_attr, be_par, bmask, d_w, d_tens, batch_map=bmap, num_graphs=n_g)
        ...
        for gamma in gamma_sweep:
            ...
            print(f"Gamma = {gamma:<10.2f} | {topo_pl:6.3f}%   | {rec:>5d}/{mwpm_errs:<4d}  | {reg:>5d}/{shots_per_dist-mwpm_errs:<6d} | {net:>+5d}")
        print()

if __name__ == "__main__":
    run_final_gate_experiment()
