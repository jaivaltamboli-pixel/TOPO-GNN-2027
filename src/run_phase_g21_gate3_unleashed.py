...
# --- 1. BYPASS ACTIVE SUBGRAPH: FULL GRAPH TENSOR BUILDER ---
def build_full_graph_tensors(s, coords, raw_edge_dict, bnd_z, bnd_x, d):
    ...
    for i, (z, x, y) in coords.items():
        ...
        x6[i, 1] = x / (d + 1)
        x6[i, 2] = y / (d + 1)
        x6[i, 3] = 1.0 if i == bnd_z else 0.0
        x6[i, 4] = 1.0 if i == bnd_x else 0.0
    ...
    for (u, v), data in raw_edge_dict.items():
        ...
        # Ensure finite attributes
        if np.isinf(w) or w > 1e4: w = 1e4 
        e_feat = [w, data.get('prob', 0.0), data.get('error_type', 0.0) / 2.0, 1.0]
...
def collect_single_distance(d, p_val, eta, shots):
    ...
    print(f"  [+] Prepared d={d} ({shots:,} shots) in {time.time()-t0:.2f}s")
    return {"samples": samples, "total_shots": shots, "preds_mwpm": preds_mwpm, "flips": flips}

def run_gate3_unleashed():
    torch.manual_seed(42); np.random.seed(42)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    p_val, eta, shots = 0.0035, 100.0, 10000

    print("=" * 90)
    print("PHASE G.21: GATE 3 UNLEASHED (FULL LATTICE + EXTENDED k-FILTER)")
    print("=" * 90 + "\n")

    train_samples = []
    # Train on 3, 5, 7. We can drop 9 for memory/speed since we use the full lattice now
    for d in [3, 5, 7]:
        dat = collect_single_distance(d, p_val, eta, shots)
        for s in dat["samples"]:
            y_true_obs = 0 if s["y_target"] == 1.0 else 1
            if y_true_obs != s["obs_A"] or abs(s["delta_w"]) <= (4.0 * d):
    ...
    for i in range(0, len(active), 64): # Smaller batch for full graphs
    ...
    # Scanning much higher k values (up to 4.5) to catch the extreme delta weights
    for k in [3.0, 3.5, 4.0, 4.5]:
