...
def run_gate3_convergence():
    torch.manual_seed(42); np.random.seed(42)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    p_val, eta, shots = 0.0035, 100.0, 10000

    print("=" * 80)
    print("PHASE G.27: THE CONVERGENCE RUN (d=5,7,9 -> EVAL d=11)")
    print("=" * 80 + "\n")

    train_samples = []
    # Drop d=3, add d=9 to bridge the zero-shot gap
    for d in [5, 7, 9]:
    ...
    optimizer = optim.AdamW(model.parameters(), lr=8e-4, weight_decay=1e-4) # Slightly lower LR for stability
    criterion = nn.MSELoss()

    # Train longer (12 epochs, 150 batches) to ensure the network actually learns the topology
    ...
    print(f"\n>>> ZERO-SHOT DISTANCE d = 11 <<<")
    print(f"{'Max dW (k*d)':<14} | {'Min Topo (tau)':<14} | {'Rec':<6} | {'Reg':<6} | {'Net'}")
    print("-" * 65)
    ...
    for k in [3.0, 3.5, 4.0]:
        for tau in [0.5, 0.75, 0.85]:
            ...
            print(f"<= {k*11.0:<11.1f} | > {tau:<12.2f} | {rec:>4d} | {reg:>4d} | {rec-reg:>+4d}")

if __name__ == "__main__":
    run_gate3_convergence()
