...
class LLRResidualRanker(nn.Module):
    ...
    def forward(self, x6, edge_index, edge_attr, is_par, mask_diff, delta_w, num_defects, batch_map=None, num_graphs=None):
        ...
        # Segmented path pooling along cycle
        ...
        # Predict log-odds residual
        global_in = torch.cat([pooled_cycle, delta_w, num_defects], dim=-1)
        residual = self.readout(global_in)

        # Full log-odds: LLR = -delta_w + neural_correction
        llr_out = -delta_w + diff_energy + residual
        return llr_out
...
def collate_cycle_batch(samples, device):
    ...
    # Convert y_val: +1 (Class 0) -> 0.0, -1 (Class 1) -> 1.0 for standard BCE with Logits
    ...
    return bx6, be_idx, be_attr, be_par, bmask, delta_w, num_defs, bmap, labels, len(samples)
...
def run_llr_ranker_experiment():
    ...
    print("=" * 95)
    print("PHASE G.7: POSTERIOR LOG-LIKELIHOOD RATIO (LLR) RANKER")
    print("=" * 95 + "\n")
    ...
    for d in train_distances:
        for s in train_data[d]["samples"]:
            if s[8] != s[9] or abs(s[6]) <= 25.0:
                train_pool.append(s)

    pool_fail = [s[:8] for s in train_pool if s[8] != s[9]]
    pool_corr = [s[:8] for s in train_pool if s[8] == s[9]]

    print(f"  Training Pool: {len(pool_fail):,d} Failures | {len(pool_corr):,d} Ambiguous Correct Shots")

    model = LLRResidualRanker(in_node_dim=6, in_edge_dim=4, hidden_dim=64, num_layers=4).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            bx6, be_idx, be_attr, be_par, bmask, d_w, n_def, bmap, labels, n_g = collate_cycle_batch(batch, device)

            optimizer.zero_grad()
            llr_pred = model(bx6, be_idx, be_attr, be_par, bmask, d_w, n_def, batch_map=bmap, num_graphs=n_g)

            loss = F.binary_cross_entropy_with_logits(llr_pred, labels)
            loss.backward()
            optimizer.step()

            total_loss += loss.item() * n_g
            total_items += n_g
            preds = (llr_pred > 0).float()
            correct_preds += (preds == labels).sum().item()

        acc = (correct_preds / total_items) * 100.0
        print(f"  Epoch {epoch:2d}/{epochs:2d} | BCE Loss: {total_loss/total_items:.4f} | Balanced LLR Acc: {acc:6.2f}%")

    print("\n[+] Evaluating LLR Decisions across Test Sets...\n")
    model.eval()

    # Beta sweeps the strength of the neural LLR relative to the zero threshold
    beta_sweep = [0.0, 0.5, 1.0, 1.5, 2.0, 3.0]
    ...
    for d in test_distances:
        ...
        for i in range(0, len(active_samples), eval_batch_size):
            ...
            with torch.no_grad():
                llr_out = model(bx6, be_idx, be_attr, be_par, bmask, d_w, n_def, batch_map=bmap, num_graphs=n_g)
        ...
        print(f"{'Decision Beta':<14} | {'Topo P_L':<10} | {'Recoveries':<11} | {'Regressions':<12} | {'Net Gain'}")
        print("-" * 75)

        for beta in beta_sweep:
            topo_preds = preds_mwpm.copy()

            # LLR > 0 implies Class 1 is more probable; LLR < 0 implies Class 0 is more probable
            for idx_k, (llr_val, obs_a, s_idx) in enumerate(zip(llr_arr, obs_A_list, shot_indices)):
                if obs_a == 0 and llr_val > beta:
                    topo_preds[s_idx] = 1
                elif obs_a == 1 and llr_val < -beta:
                    topo_preds[s_idx] = 0
            ...
            print(f"Beta = {beta:<8.2f} | {topo_pl:6.3f}%   | {rec:>5d}/{mwpm_errs:<4d}  | {reg:>5d}/{shots_per_dist-mwpm_errs:<6d} | {net:>+5d}")
        print()

if __name__ == "__main__":
    run_llr_ranker_experiment()
