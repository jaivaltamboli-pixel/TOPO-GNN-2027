...
class TopoOracle(nn.Module):
    ...
    def forward(self, x6, edge_index, edge_attr, is_par, mask_diff, batch_map=None, num_graphs=None):
        ...
        # Scale by sqrt(len) to prevent signal decay on large-diameter graphs
...
def run_receptive_fix():
    torch.manual_seed(42)
    np.random.seed(42)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")

    p_val, eta = 0.0035, 100.0
    shots = 12000

    print("=" * 95)
    print("PHASE G.19: RECEPTIVE FIELD & SQRT-NORM FIX (TRAIN d=3,5,7,9 -> EVAL d=11)")
    print("=" * 95 + "\n")
    ...
    active_samples = test_d11["samples"]
    preds_mwpm = test_d11["preds_mwpm"]
    flips = test_d11["flips"]
