...
# -------------------------------------------------------------------------
# 1. ARCHITECTURE WITH CYCLE & GLOBAL ENTROPY READOUT
# -------------------------------------------------------------------------
...
class PrecisionCycleRanker(nn.Module):
    def __init__(self, in_node_dim=6, in_edge_dim=4, hidden_dim=64, num_layers=4):
        ...
        # Edge bias for the cycle
        self.edge_scorer = nn.Sequential(
            nn.Linear(hidden_dim * 2 + in_edge_dim + 1, hidden_dim),
            nn.GELU(),
            nn.Linear(hidden_dim, 1)
        )
        # Global entropy correction head (combines cycle pooling + scalar Delta W)
    ...
    def forward(self, x6, edge_index, edge_attr, is_par, mask_diff, delta_w, num_defects, batch_map=None, num_graphs=None):
        ...
        # Pool features along the divergent cycle
        ...
        # Cycle edge counts for normalization
...
# -------------------------------------------------------------------------
# 3. ASYMMETRIC TRAINING & BENCHMARK
# -------------------------------------------------------------------------
def run_phase_g4():
    ...
    print("=" * 95)
    print("PHASE G.4: ASYMMETRIC PRECISION-CALIBRATED CYCLE RANKER")
    print(f"Goal: Eliminate false regressions by conditioning on Delta W and enforcing precision margins")
    print("=" * 95 + "\n")
    ...
    # High-signal training pool: error pairs (where MWPM picked wrong) vs correct boundary pairs
    pool_mwpm_fail = [s[:8] for s in train_pool if (s[8] != s[9])]
    pool_mwpm_corr = [s[:8] for s in train_pool if (s[8] == s[9] and abs(s[6]) < 15.0)]

    print(f"  Critical Pool: {len(pool_mwpm_fail):,d} MWPM Failures | {len(pool_mwpm_corr):,d} Borderline MWPM Correct Shots")
    ...
    for epoch in range(1, epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            idx_fail = np.random.choice(len(pool_mwpm_fail), half_b, replace=True)
            idx_corr = np.random.choice(len(pool_mwpm_corr), half_b, replace=True)
            batch = [pool_mwpm_fail[i] for i in idx_fail] + [pool_mwpm_corr[j] for j in idx_corr]
            ...
            # Asymmetric margin loss: heavily penalize flipping when MWPM was already correct
            margin = targets * energy
        ...
        print(f"  Epoch {epoch:2d}/{epochs:2d} | Precision Margin Loss: {total_loss/total_items:.4f} | Mining Accuracy: {acc:6.2f}%")

    print("\n[+] Evaluating across test sets...\n")
    model.eval()

    # Delta W boundary gating: Only evaluate GNN arbitration if Delta W <= max_gap
    # When Delta W is large, MWPM is virtually 100% correct
    gap_thresholds = [4.0, 8.0, 12.0, 16.0, 24.0]
    ...
    for d in test_distances:
        ...
        print(f"{'Max Delta W':<12} | {'Topo P_L':<10} | {'Recoveries':<11} | {'Regressions':<12} | {'Net Gain':<10} | {'Altered'}")
        print("-" * 80)

        for max_g in gap_thresholds:
            topo_preds = preds_mwpm.copy()

            for idx_k, (e_val, dw_val, obs_a, s_idx) in enumerate(zip(e_arr, d_w_arr, obs_A_list, shot_indices)):
                # Only arbitrate if within the classical ambiguity window
                if abs(dw_val) <= max_g:
                    if obs_a == 0 and e_val < -2.0:
                        topo_preds[s_idx] = 1
                    elif obs_a == 1 and e_val > 2.0:
                        topo_preds[s_idx] = 0
            ...
            print(f"{max_g:<12.1f} | {topo_pl:6.3f}%   | {rec:>5d}/{mwpm_errs:<4d}  | {reg:>5d}/{shots_per_dist-mwpm_errs:<6d} | {net:>+5d}     | {altered:>5d}")
        print()
