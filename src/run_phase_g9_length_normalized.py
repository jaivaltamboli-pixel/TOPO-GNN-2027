...
class NormalizedLLRRanker(nn.Module):
    ...
    def forward(self, x6, edge_index, edge_attr, is_par, mask_diff, delta_w, num_defects, d_val, batch_map=None, num_graphs=None):
        ...
        # Count active cycle edges per graph
        ...
        # Mean cycle energy rate (independent of path length)
        diff_energy_sum = torch.zeros((num_graphs, 1), device=h.device)
        diff_energy_sum.index_add_(0, batch_map[src], edge_bias * mask_diff)
        mean_diff_energy = diff_energy_sum / cycle_counts

        # Cycle pooled representation
        ...
        # Distance-normalized inputs
        norm_dw = delta_w / d_val
        norm_def = num_defects / (d_val ** 2)

        global_in = torch.cat([pooled_cycle, norm_dw, norm_def], dim=-1)
        residual = self.readout(global_in)

        # Normalized LLR decision metric
        llr_norm = -norm_dw + mean_diff_energy * 2.5 + residual
        return llr_norm

def collate_cycle_batch(samples, device):
    x6_list, e_idx_list, e_attr_list, e_par_list, mask_diff_list = [], [], [], [], []
    batch_map, y_list, delta_w_list, n_def_list, d_list = [], [], [], [], []
    node_offset = 0

    for i, item in enumerate(samples):
        x6, e_idx, e_attr, e_par, mask_diff, y_val, delta_w, n_def, d_val = item
    ...
    labels = torch.tensor([1.0 if y < 0 else 0.0 for y in y_list], dtype=torch.float32, device=device).unsqueeze(-1)
    ...
    return bx6, be_idx, be_attr, be_par, bmask, delta_w, num_defs, d_tens, bmap, labels, len(samples)
...
def collect_dataset(distances, p_val, eta, shots):
    ...
    for d in distances:
        ...
        for idx in active_indices:
            ...
            samples.append((x6, e_idx.long(), e_attr, (e_par.view(-1) > 0).float(), mask_diff, y_target, delta_w, n_def, float(d), obs_A, y_true, idx))
...
def run_length_normalized_experiment():
    ...
    print("=" * 95)
    print("PHASE G.9: LENGTH-NORMALIZED LLR DECODER")
    print("=" * 95 + "\n")
    ...
    for d in train_distances:
        for s in train_data[d]["samples"]:
            if s[9] != s[10] or abs(s[6]) <= (3.0 * d):
                train_pool.append(s)

    pool_fail = [s[:9] for s in train_pool if s[9] != s[10]]
    pool_corr = [s[:9] for s in train_pool if s[9] == s[10]]

    print(f"  Training Pool: {len(pool_fail):,d} Failures | {len(pool_corr):,d} Ambiguous Correct Shots")

    model = NormalizedLLRRanker(in_node_dim=6, in_edge_dim=4, hidden_dim=64, num_layers=4).to(device)
    ...
    for epoch in range(1, epochs + 1):
        ...
        for _ in range(steps_per_epoch):
            ...
            bx6, be_idx, be_attr, be_par, bmask, d_w, n_def, d_tens, bmap, labels, n_g = collate_cycle_batch(batch, device)

            optimizer.zero_grad()
            llr_norm = model(bx6, be_idx, be_attr, be_par, bmask, d_w, n_def, d_tens, batch_map=bmap, num_graphs=n_g)

            loss = F.binary_cross_entropy_with_logits(llr_norm, labels)
            loss.backward()
            optimizer.step()

            total_loss += loss.item() * n_g
            total_items += n_g
            preds = (llr_norm > 0).float()
            correct_preds += (preds == labels).sum().item()

        acc = (correct_preds / total_items) * 100.0
        print(f"  Epoch {epoch:2d}/{epochs:2d} | BCE Loss: {total_loss/total_items:.4f} | Balanced Accuracy: {acc:6.2f}%")

    print("\n[+] Evaluating Length-Normalized LLR Decisions across Test Sets...\n")
    model.eval()

    # Uniform threshold sweep across all code sizes
    gamma_sweep = [0.0, 0.5, 1.0, 1.5, 2.0, 2.5, 3.0, 4.0]
    ...
    for d in test_distances:
        ...
        eval_batch_size = 256
        llr_list, obs_A_list, shot_indices = [], [], []
        ...
        for i in range(0, len(active_samples), eval_batch_size):
            ...
            bx6, be_idx, be_attr, be_par, bmask, d_w, n_def, d_tens, bmap, _, n_g = collate_cycle_batch(batch_inputs, device)

            with torch.no_grad():
                llr_out = model(bx6, be_idx, be_attr, be_par, bmask, d_w, n_def, d_tens, batch_map=bmap, num_graphs=n_g)
                llr_list.extend(llr_out.cpu().numpy().flatten())

            for item in batch_raw:
                obs_A_list.append(item[9])
                shot_indices.append(item[11])

        llr_arr = np.array(llr_list)

        print(f">>> DISTANCE d = {d:2d} {'(ZERO-SHOT)' if d==9 else ''} (MWPM P_L: {mwpm_errs/shots_per_dist*100:.3f}%) <<<")
        print(f"{'Threshold Gamma':<18} | {'Topo P_L':<10} | {'Recoveries':<11} | {'Regressions':<12} | {'Net Gain'}")
        print("-" * 80)

        for gamma in gamma_sweep:
            topo_preds = preds_mwpm.copy()

            for idx_k, (llr_val, obs_a, s_idx) in enumerate(zip(llr_arr, obs_A_list, shot_indices)):
                if obs_a == 0 and llr_val > gamma:
                    topo_preds[s_idx] = 1
                elif obs_a == 1 and llr_val < -gamma:
                    topo_preds[s_idx] = 0
            ...
            print(f"Gamma = {gamma:<10.1f} | {topo_pl:6.3f}%   | {rec:>5d}/{mwpm_errs:<4d}  | {reg:>5d}/{shots_per_dist-mwpm_errs:<6d} | {net:>+5d}")
        print()

if __name__ == "__main__":
    run_length_normalized_experiment()
