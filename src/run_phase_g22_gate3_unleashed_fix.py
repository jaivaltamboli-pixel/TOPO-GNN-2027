...
# --- 1. FULL GRAPH TENSOR BUILDER (FIXED BOUNDARY ALLOCATION) ---
def build_full_graph_tensors(s, coords, raw_edge_dict, bnd_z, bnd_x, d):
    # FIX: Dynamically size to the max index found in the edges to guarantee boundaries are included
    ...
    for i, (z, x, y) in coords.items():
        ...
        x6[i, 1] = x / (d + 1)
        x6[i, 2] = y / (d + 1)
        if i < len(s):  # Safe check
            x6[i, 5] = 1.0 if s[i] else 0.0
            
    # Set boundary flags safely
...
def run_gate3_unleashed():
    ...
    print("=" * 90)
    print("PHASE G.22: GATE 3 UNLEASHED FIX (CUDA BOUNDARY RESOLVED)")
    print("=" * 90 + "\n")
