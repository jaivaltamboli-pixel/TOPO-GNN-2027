...
# --- 1. FULL GRAPH TENSOR BUILDER (WITH SAFE EDGE NORM) ---
def build_full_graph_tensors(s, coords, raw_edge_dict, bnd_z, bnd_x, d):
    max_idx = max(max(u, v) for u, v in raw_edge_dict.keys())
    num_nodes = max_idx + 1
    
    x6 = torch.zeros((num_nodes, 6), dtype=torch.float32)
    for i, (z, x, y) in coords.items():
        x6[i, 0] = z / (d + 1)
        x6[i, 1] = x / (d + 1)
        x6[i, 2] = y / (d + 1)
        if i < len(s):  
            x6[i, 5] = 1.0 if s[i] else 0.0
            
    if bnd_z < num_nodes: x6[bnd_z, 3] = 1.0
    if bnd_x < num_nodes: x6[bnd_x, 4] = 1.0
    ...
    for (u, v), data in raw_edge_dict.items():
        ...
        # FIX 3: Gradient-safe normalization for extreme weights (clips inf to 50, scales to ~O(1))
        w_norm = min(w, 50.0) / 10.0 
    ...
    e_attr = torch.tensor(attr, dtype=torch.float32)
    e_par = torch.tensor(par, dtype=torch.float32)
    return x6, e_idx, e_attr, e_par, gp
...
def collect_single_distance(d, p_val, eta, shots):
    ...
    for idx in active_indices:
        ...
        # FIX 4: PyMatching Tie-Breaker Alignment
        if obs_A != batch_pred:
        ...
        x6, e_idx, e_attr, e_par, global_pairs = build_full_graph_tensors(s, coords, raw_edge_dict, bnd_z, bnd_x, d)
        ...
        samples.append({"x6": x6, "e_idx": e_idx, "e_attr": e_attr, "e_par": (e_par.view(-1) > 0).float(),
                        "mask_diff": mask_diff, "y_target": 1.0 if y_true == 0 else -1.0, 
                        "delta_w": w_1 - w_0, "d_val": float(d), "obs_A": obs_A, "idx": idx})
                        
    print(f"  [+] Prepared d={d} ({shots:,} shots) in {time.time()-t0:.2f}s | Dropped due to tie-break mismatch: {skipped_api_mismatch}")
    return {"samples": samples, "total_shots": shots, "preds_mwpm": preds_mwpm, "flips": flips}

def run_gate3_calibrated():
    torch.manual_seed(42); np.random.seed(42)
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    p_val, eta, shots = 0.0035, 100.0, 10000

    print("=" * 90)
    print("PHASE G.23: STRICT CALIBRATION (SYMMETRIC FILTERS + ALIGNED WEIGHTS)")
    print("=" * 90 + "\n")
    ...
    for d in [3, 5, 7]:
        ...
        for s in dat["samples"]:
            # FIX 1 & 2: Strictly Symmetric Filtering
            # Both Fails and Corrects must pass the SAME ambiguity threshold
    ...
    # We maintain 50/50 balance in training to prevent classical mode collapse, 
    # but the data populating those pools is now strictly sanitized.
    for epoch in range(1, 10):
        ...
        print(f"  Epoch {epoch:2d}/9 | Loss: {tot_loss/items:.4f} | Acc: {(correct/items)*100:6.2f}%")
