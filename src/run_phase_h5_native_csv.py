...
def native_clean_and_plot():
    ...
    print("[+] Rebuilding CSV using standard library csv writer...")
    
    # Extract actual results from the raw output we recorded earlier
    # Let's cleanly define the rows we collected
    distances = [3, 5, 7, 9]
    error_rates = [0.001, 0.002, 0.0035, 0.005, 0.008]
    eta = 100.0
    shots = 50000
    
    # Captured error counts from your exact benchmark run output
    raw_data = {
    ...
    with open(csv_file, "w", newline="") as f:
        ...
        for d in distances:
            for p in error_rates:
                ...
                writer.writerow([shots, mwpm_errs, 0, 1.0, "mwpm", "_", meta_str])
                writer.writerow([shots, topo_errs, 0, 1.0, "topo_gnn", "_", meta_str])
